...
namespace OpenBox {
    public class MagicaFile {
        ...
        static Chunk ReadChunk(BinaryReader br) {
            Chunk chunk = new Chunk();
            chunk.chunkId = BytesToString(br.ReadBytes(4));
            chunk.numBytes = br.ReadInt32();
        ...
        public static VoxelSet<Vec4b>[] Load(string filename) {
            ...
            Console.WriteLine("Number of models: {0}", numModels);
            VoxelSet<Vec4b>[] models = new VoxelSet<Vec4b>[numModels];

            // Read all models
            for (int i = 0; i < numModels; ++i) {
                Chunk sizeChunk = ReadChunk(br);
                if (sizeChunk.chunkId != kChunkSize) {
                    throw new Exception("Bad size chunk");
                }

                // TODO: Need to swap xyz for y-up
                //Vec3i size = new Vec3i(br.ReadInt32(), br.ReadInt32(), br.ReadInt32());
                Vec3i size = new Vec3i();
                size.x = br.ReadInt32();
                size.z = br.ReadInt32();
                size.y = br.ReadInt32();
                models[i] = new VoxelSet<Vec4b>(size);

                Chunk idxChunk = ReadChunk(br);
                ...
                for (int j = 0; j < numVoxels; ++j) {
                    ...
                    models[i][idx] = new Vec4b(br.ReadByte());
                }
            }

            Vec4b[] colors = new Vec4b[256];

            // (Optional) Read palette
            if (br.PeekChar() == 'R') {
                Chunk palChunk = ReadChunk(br);
